...
```html
<fui-notice-bar content="这是一条通告消息，请注意查看。"></fui-notice-bar>
```
...
```html
<fui-notice-bar content="带图标的通告栏">
  <fui-icon slot="left" name="notification" size="36"></fui-icon>
  <fui-icon slot="right" name="close" size="36" bind:click="closeNotice"></fui-icon>
</fui-notice-bar>
```

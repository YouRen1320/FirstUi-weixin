...
```html
<!-- 自动上传 -->
<fui-upload immediate url="https://example.com/upload" bindsuccess="success"></fui-upload>
```

## 手动上传与回显

```html
<fui-upload fileList="{{fileList}}" url="{{url}}" id="upload" bindsuccess="success"></fui-upload>
<fui-button text="开始上传" bindclick="startUpload"></fui-button>
```
...
<fui-upload max="{{3}}" size="{{2}}" url="{{url}}"></fui-upload>
```
